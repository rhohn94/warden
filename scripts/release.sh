...
run_release() {
    ...
    # Warden adaptation: the verify build (cargo) rewrites Cargo.lock's
    # own-package version stanza after the Cargo.toml bump — stage it with the
    # bump commit so the ceremony ends on a clean tree. (Never bumped by regex:
    # that could clobber a dependency pin; cargo maintains the lockfile.)
    if [ -f Cargo.lock ] && ! git diff --quiet -- Cargo.lock 2>/dev/null; then
        present_files+=("Cargo.lock")
    fi
