...
loop-state file via `advance_loop`); the AGENT commits. Design: the
grimoire-release-server-design.md framework spec, which lives only in the
upstream Grimoire repository, not in this project's own docs/.
...
def _bootstrap_imports(repo_root: pathlib.Path) -> None:
    ...
    plan_candidates = [
        repo_root / ".claude" / "skills" / "grm-release-agent-tracker",
        repo_root / "scripts",
    ]
    loop_candidates = [
        repo_root / ".claude" / "skills" / "grm-noir-loop",
        repo_root / "scripts",
    ]
