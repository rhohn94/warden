---
name: grm-status-broker
description: Dedicated own-session, strictly read-only agent that answers "what is the status of X?" cheaply by exhausting structured sources before touching code — ordered lookup (1) issue tracker, (2) documentation, (3) source code only as a last resort. No git writes, no issue-tracker writes; narrow context. Use when the user wants a project status, overview, or "where are we on X".
---
...
>
> ```
> python3 .claude/skills/grm-status-broker/project_status.py --root .
> ```
>
...
`python3 .claude/skills/grm-status-broker/version_history.py --release vX.Y`
