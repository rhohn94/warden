...
description: Run 14 deterministic checks over a Grimoire repo's own documentation: flavor-parity (claude-code/root/copilot drift), design-doc house-layout, internal-link integrity, a validated docs map, cross-doc release consistency, manifest-detect-hygiene, shipped-pointers, skill-budget, relative-links (absolute-internal + broken anchors + bare-prose), hierarchy (reachability + breadcrumbs + per-tier index), lean-index, monolith-cap, description-cap, and anti-patterns. Report-only unless --strict. Use when checking doc quality or validating the docs.
---

# doc-assurance

Self-checking pass over Grimoire's own docs. One script, 14 checks. Run at
