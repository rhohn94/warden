...
`protected-branch-guard.sh`, and `version/{X.Y}` is protected. If the
...
`docs/grimoire/integration-workflow.md` §Git-protocol governance.
...
- Run `grm-release-phase` to spawn Phase 1 work-item sessions (via `spawn_task`).
...
- The `grm-worktree-preflight` skill checks the merge base; a worktree branched off
...
`grm-release-planning` skill Step 3) carry the **never-silently-trimmed** guarantee,
