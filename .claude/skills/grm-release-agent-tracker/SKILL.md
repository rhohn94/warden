---
name: grm-release-agent-tracker
description: Track the status of all subagent branches for the in-flight release, mark agents done when the user reports back, and determine what is ready to merge. Source of truth is the §5 ledger in the agreed release plan. Use when the user says an agent is done, or to check agent status / the current merge queue.
---
...
> .claude/skills/grm-release-agent-tracker/release_plan.py {get-ledger|diff|merge-queue}`
...
recommend running `grm-release-phase-merge`.
...
- It does not merge branches. Use `grm-release-phase-merge` for that.
- It does not run tests. Tests run during `grm-release-phase-merge`.
- It does not tick ☑ Merged — that happens inside `grm-release-phase-merge` after
