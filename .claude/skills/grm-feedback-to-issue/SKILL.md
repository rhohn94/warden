---
name: grm-feedback-to-issue
description: Convert freeform feedback (a user/beta-tester comment, a noticed bug, a review note, an agent observation) into a well-formed normalized IssueDraft and file it via the issue-tracker abstraction. Sets audience so routing resolves automatically — no tracker name required. Use when converting feedback into an issue or filing a noticed bug.
---
...
3. **Planning consequence** — tagged issues are **always-prioritized origin-D
   items** in `grm-release-planning` (WEB-6); they may be scheduled across versions
