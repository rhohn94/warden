...
`docs/grimoire/design/managed-project-tooling-design.md`.
...
3. **Section B — complexity + maintainability.** Run the complexity tool; collect
   ...
   import scan (the same scan `grm-architecture-audit` uses): afferent coupling
...
- Read-only except `--accept` (writes the baseline cache) and `--file-issues`
  (routes through `grm-feedback-to-issue`, deduped).
