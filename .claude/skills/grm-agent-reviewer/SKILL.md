---
name: grm-agent-reviewer
description: Dedicated own-session pre-merge auditor that reads a completed branch/diff and returns findings split into blocking/non-blocking items. Wraps `code-review`. Distinct from the Verifier (runs tests/build/release) and QA agent (retrospective, post-ship) — reads the diff only. Use to spawn a reviewer or review a branch pre-merge.
---
...
Design rationale (§B Reviewer per-role contract §B.RV1, §C spawn/return
contract) lives in the upstream Grimoire repository (framework-internal — not
shipped).

**See also:** `grm-agent-verifier` (tests/build/release, pass/fail) and
`grm-agent-qa` (retrospective, post-ship) — the Reviewer only reads the diff.
...
demand before a specific merge.
