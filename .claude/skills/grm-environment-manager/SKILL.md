...
|---|---|---|
| What is listening? | `list_processes` | `python3 .claude/skills/grm-environment-manager/env_probe.py` |
| What holds port X? | `port_status {"ports":[X]}` | `python3 .claude/skills/grm-environment-manager/env_probe.py --port X` |
| Is `<app>` running? | `instance_urls {"name":"<app>"}` | `python3 .claude/skills/grm-environment-manager/env_probe.py --name <app>` |
...
commit or file directly — it **dispatches a Reporter** (`grm-reporter` skill /
