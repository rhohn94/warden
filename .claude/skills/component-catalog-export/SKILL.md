---
name: component-catalog-export
description: Scan a project's reusable components and emit a machine- and human-readable catalog report (each component's id, summary, profile tags, provides/requires, compatibility signals, stability). Designed for downstream Grimoire consumers — not just maintainers — to discover what's available and to author/update quick-start templates. Read-only. Triggers on "export the component catalog", "list available components", "component catalog", "what components are available", "catalog the shared components", "generate the component catalog report".
---

# Component-catalog export

Produces a catalog of the project's reusable components — the input a
`quick-start-template` author consumes, and a discovery aid for anyone bootstrapping
...
versioned successor is the `component-registry` skill (`.claude/component-registry.json`,
...
>
> ```bash
> python3 .claude/skills/component-registry/component_registry.py dry-run --stdout
> ```
>
> `dry-run` computes the registry **without writing** (this export is read-only —
> it never mutates `.claude/component-registry.json`; the `component-registry`
...
- Treating the export as the catalog system — it's a stopgap / view; the
  versioned source of truth is `component-registry` (build it with that skill).
- Rebuilding or mutating `.claude/component-registry.json` from this export — it
  is read-only here; the `component-registry` skill owns the registry.
