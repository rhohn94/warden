...
description: Dedicated own-session, strictly read-only agent that investigates a bounded question and returns a condensed structured brief — without polluting the requester's context. Wraps `Explore` / `deep-research`; no file writes, no git, no issue-tracker writes. Use when the user wants to spawn a scout or gather context/a brief before planning or committing — not for a status lookup ("what is the status of X"), which is grm-agent-status-broker's job.
---

# Scout agent (SC1)

**See also:** `grm-agent-status-broker` — use the Scout for open-ended
investigation ("how does X work", prior art); use the status-broker for a
"what is the status of X" lookup (issue tracker → docs → code).
...
Design authority: `docs/grimoire/design/agent-roles-design.md` §B.5 (Scout contract),
...
- During `grm-release-planning`: the roadmap contains an item whose scope depends on
