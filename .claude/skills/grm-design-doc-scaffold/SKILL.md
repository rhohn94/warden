---
name: grm-design-doc-scaffold
description: Scaffold a new docs/design/{feature}-design.md with the house section layout. Use when a feature task lacks a design doc.
---
...
3. Confirm you are on a work branch off `dev` / `version/{X.Y}`, not on `dev`
   or `main` directly. Run the **`grm-worktree-preflight`** skill first if you are
   unsure.
