...
description: Switch the active execution-strategy (the workflow-variant dial) by validating workflow-variant.value against the preset set {Fast, Efficient, Cheap-Slow} and writing it to .claude/grimoire-config.json. Idempotent — exits early if the requested strategy is already active. The integration master reads the field live at dispatch time, so no file-swap is performed. Migrates the legacy Careful-Serial preview value to Cheap-Slow, and drops a legacy in-development preview flag. Use when onboarding picks a dispatch posture, when the user wants a different execution strategy, or to graduate a legacy preview config. Triggers on "switch execution strategy", "change dispatch posture", "set workflow variant", "use Cheap-Slow", "switch to Fast dispatch".
---
