---
name: ux-demo-build
description: Build (or refresh) a minimal ux-demo/ app in the project's own tech stack to verify the project's design-language adaptation. Opt-in only — not auto-maintained. Use after the first design-language-adapt to produce a demo for user review, or on demand when the adaptation has changed. Triggers on "build the ux-demo", "refresh the ux-demo", "verify the design language adaptation", "rebuild the demo app".
---
...
correctly in real project code. Run once after the first `design-language-adapt`,
...
- **Primary moment:** right after `design-language-adapt` completes its first
...
- **Never** invoke this skill automatically — not from `design-language-adapt`,
...
`workflow-bootstrap`'s GUI interview), or ask the user before proceeding.
...
Or invoke the `feedback-to-issue` skill directly. The abstraction routes to the
configured tracker automatically (roadmap backend if no `issue-tracker` block is
...
- **Building a full design-language implementation.** The demo is minimal proof,
  ...
  creep.
- **Auto-running from `design-language-adapt`.** The adapt skill may suggest
...
- **Directly appending UX issues to `docs/roadmap.md ## Backlog`.** Route UX
  issues through `feedback-to-issue` so they land in the configured tracker.
