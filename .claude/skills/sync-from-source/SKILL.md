...
description: Pull workflow skills, hooks, and structural docs from a source project (the repo where you actively develop the workflow) into this Grimoire repo, then re-generalize and refresh the golden baseline. Safe by default — never overwrites newer or in-progress scaffolding files without review. Use when you've improved a skill/hook/doc in your main project and want those improvements reflected in the reusable scaffolding. Triggers on "sync from <project>", "pull workflow changes from my repo", "import skills from source", "update the scaffolding from forge-engine", "bring over my latest skills".
---
...
`workflow-bootstrap` does. This skill only pulls inbound.
...
```bash
.claude/skills/sync-from-source/sync-from-source.sh <source-path> --diff
# or:  SCAFFOLD_SOURCE=<source-path> .claude/skills/sync-from-source/sync-from-source.sh --diff
```
...
| `SRC-MISSING` | Scaffolding-only skill (e.g. you renamed). | Expected for `source-to-design-docs` etc. Ignore. |
...
`project-release` maps to a source's `forge-release`). Update it if your source
...
- **Templates are never overwritten by real content.** `docs/roadmap.md` and
  ...
  external provider like GitHub Issues) — see `issue-tracker-switch` for the
...
```bash
.claude/skills/sync-from-source/sync-from-source.sh <source-path> --apply
```
...
restore baseline so `workflow-bootstrap` reproduces them:

- Run the **`workflow-snapshot`** skill. Its mandatory genericize check is a
...
- Skipping Step 4 — live skills updated but `golden/` left stale means a future
  `workflow-bootstrap` restore silently reverts your improvements.
