...
python3 .claude/skills/dependency-audit/dependency_channel_conformance.py --self-test

# Audit a repo (checks 1 & 2 offline; check 3 needs `gh`):
python3 .claude/skills/dependency-audit/dependency_channel_conformance.py --root . --json
python3 .claude/skills/dependency-audit/dependency_channel_conformance.py --root . --offline
```

Exit 0 = conformant, nonzero = at least one violation (the loud-exit contract).
**Warn-only this release:** the merge-gate (`release-phase-merge` §3.5 step 3a)
runs it advisory — reads the live `code-quality.audit-gate` dial, files findings
via `feedback-to-issue` (labels `security` + `dependency-channel`), and proceeds.
