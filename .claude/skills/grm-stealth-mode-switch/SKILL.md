...
6. **Snapshot the branch baseline:**
   `python3 .claude/skills/grm-stealth-mode-switch/stealth_scrub.py branches --baseline`
7. **Flip the config** atomically (temp + validate + replace): set
   `stealth-mode.value:"on"`, `acknowledged-risk:true`, `schema-version:4`. Then
   `python3 .claude/skills/grm-config-validate/config_validate.py` must pass clean.
...
- **Cross-rule enforced:** `value:"on"` with `acknowledged-risk:false` is a hard
  `grm-config-validate` error — stealth cannot be activated by hand-editing the
