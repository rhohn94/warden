...
Design rationale (§B.7 Triager contract, §C spawn + return, §A taxonomy table)
...
Grimoire repository for that rationale). These hard rules apply in **all
paradigms** and cannot be overridden by the spawn-prompt bounds:
...
Design rationale (§6.2 never-silently-trimmed rule; §Protected framework labels
of the label-taxonomy doc) lives in the upstream Grimoire repository
