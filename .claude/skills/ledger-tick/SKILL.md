...
> `python3 .claude/skills/release-agent-tracker/release_plan.py get-ledger` and
...
1. **Locate the active plan.** `ls docs/release-planning-v*.md` — pick the
   highest unreleased version. If you are unsure which is active, check
   `docs/version-history.md` for the most recently *released* version; the
   active plan is the next one up.
...
6. **Commit — once per sweep, not per branch.** When ticking multiple branches
   from a single merge sweep (the common case under `release-phase-merge`), flip
