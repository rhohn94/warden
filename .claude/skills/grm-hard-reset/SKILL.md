...
| `--reonboard-now` | Run the `grm-onboarding` interview inline after the reset instead of arming the sentinel for the next prompt (Step 8). | off (arm sentinel) |
...
working tree. The framework inventory is sourced from `grm-workflow-bootstrap`'s
...
2. **Restore framework to pristine.** Run `workflow-bootstrap --restore` semantics
   ...
   subsequent `grm-work-paradigm-switch`.)
