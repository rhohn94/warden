---
name: grm-agent-verifier
description: Dedicated own-session QA agent that checks a completed work-item branch against its acceptance criteria — running tests, build, and release commands — and returns a structured pass/fail verdict. Distinct from the Reviewer (code-diff read, no commands) and the QA agent (retrospective, post-ship) — not the task agent (self-reviews). Use to spawn a verifier or run independent pre-merge QA on a branch.
---
...
Design authority: `docs/grimoire/design/agent-roles-design.md` §B (Verifier contract),
§C (spawn/return contract).

## See also

- **Reviewer** (`grm-agent-reviewer`) — reads the code diff pre-merge and
  returns blocking/non-blocking findings; never executes tests/build/release
  commands. Use the Reviewer for a code-quality read, the Verifier for a
  runtime pass/fail check against acceptance criteria.
- **QA agent** (`grm-agent-qa`) — retrospective and post-ship: it audits
  already-shipped features against their original acceptance criteria across a
  release window, and files gaps itself. The Verifier is pre-merge, one branch
  at a time, and does not write to the issue tracker.
...
work-item branch before `grm-release-phase-merge` integrates it.
...
integration master, and the Reporter. Canonical taxonomy table (all roles):
`grm-agent-scout` §7 — see it there rather than here to avoid drift between copies.
...
automatically. Full taxonomy doc: `docs/grimoire/integration-workflow.md` §Agent-type
taxonomy.
...
- Under **Supervised / Weiss**: the integration master presents the verdict to the
  ...
  fix) or to file the failure as an issue via `grm-feedback-to-issue` or a Reporter.
