---
name: grm-release-phase-model-switch
description: Switch the active release-phase-model dial ({Default, Auto}) and write it to .claude/grimoire-config.json. Idempotent. Auto is Noir-only and fails closed unless work-paradigm.value == "Noir". Use when onboarding picks a release-execution model, when a Noir project wants fully in-session orchestration, or to revert to the Default spawn_task pipeline.
---
...
> Like `grm-workflow-variant-switch` and `grm-model-effort-profile-switch`, this skill
> performs **no file-swap**. The release-phase model is pure data: the consumer
> (the integration master — `grm-release-phase` / `grm-release-phase-merge`) reads
> `release-phase-model.value` from config **live** at execution time. Writing the
> config field IS the activation. (Contrast `grm-work-paradigm-switch`, which
...
- Leave `schema-version` and every other field unchanged. The block is
  **additive and optional** — adding or switching it does **not** bump
  ...
  `grm-issue-tracker` additive-field precedent; see design §Idempotency).
...
- **Default** — the master decomposes the plan into phases and dispatches each
  work item as a separate session (`grm-release-phase` spawn_task chips), merging
  each branch via `grm-release-phase-merge`. Exactly today's pipeline; all paradigms.
...
- The integration master (`grm-integration-master` / `grm-release-phase` /
  `grm-release-phase-merge`) reads `release-phase-model.value` live at execution time
