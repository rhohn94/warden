---
name: grm-agent-environment-manager
...
|---|---|---|
| What is listening? | `list_processes` | `python3 .claude/skills/grm-agent-environment-manager/env_probe.py` |
| What holds port X? | `port_status {"ports":[X]}` | `python3 .claude/skills/grm-agent-environment-manager/env_probe.py --port X` |
| Is `<app>` running? | `instance_urls {"name":"<app>"}` | `python3 .claude/skills/grm-agent-environment-manager/env_probe.py --name <app>` |
...
after a crash), then **wait for explicit per-action authorization**. Once
authorized, kill via the standard `stop` recipe
(`python3 .claude/skills/grm-build-recipe/recipe.py stop --port <p>`, or `just
stop port=<p>`, RSS-4 #322) rather than hand-rolled `kill`/`pkill` — its
resolution order (port → `$GRIMOIRE_APP_PORT` → the pidfile `run` wrote → a
declared process pattern) only kills processes it can positively identify as
this project's own, and is idempotent (nothing running is exit 0). Re-probe to
...
**Use the worktree's claimed port.** In a parallel-worktree dispatch, never
...
claim one first with `grm-worktree-preflight`'s `claim_port.py`
...
commit or file directly — it **dispatches a Reporter** (`grm-agent-reporter` skill /
...
- Hand-rolling `kill`/`pkill` instead of the standard `stop` recipe (`recipe.py stop` / `just stop`).
