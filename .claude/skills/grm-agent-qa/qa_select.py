...
`docs/version-history.md` (the release list), `docs/grimoire/qa-ledger.md` (which
releases are already QA-verified), and each
`docs/release-planning/release-planning-vX.Y.md` (the per-feature Status Ledger
+ acceptance sources; pre-v3.45 archive and flat-path locations are tried as
fallback — see `build_worklist()`) — and emits a single JSON work-list naming
the target release(s) and the per-feature items to check. The agent then does
the judgement (read code/tests, decide pass/fail) and the filing.
...
State: a release is "verified" iff `docs/grimoire/qa-ledger.md` has a row for it whose
...
# The regex pattern matching both two-part (vX.Y) and three-part (vX.Y.Z)
# versions is canonical in project_status.py (fixed under #141).
...
def parse_ledger(ledger_text: str | None) -> dict:
...
def parse_status_ledger(planning_text: str | None) -> list:
...
def acceptance_sources(planning_text: str | None, root: str) -> list:
    """Collect referenced design docs (acceptance-criteria sources) from a
    release-planning doc — any docs/design/*.md or docs/grimoire/design/*.md
    path it mentions. v3.39 "Bulkhead": framework design docs relocated to
    docs/grimoire/design/, so a release-planning doc may reference acceptance
    sources at either tier; both are collected."""
    out = []
    if planning_text:
        for m in re.finditer(
            r"((?:docs/grimoire/design|docs/design)/[A-Za-z0-9_\-]+\.md)",
            planning_text,
        ):
            rel = m.group(1)
            if rel not in out:
...
def build_worklist(
    ...
    ledger_text = _read(os.path.join(root, "docs", "grimoire", "qa-ledger.md"))
    ledger_status = parse_ledger(ledger_text)
    ledger_present = ledger_text is not None
    if ledger_present:
        sources_read.append("docs/grimoire/qa-ledger.md")
    else:
        degraded.append("docs/grimoire/qa-ledger.md (missing — all releases treated as open targets)")
    ...
    return {
        ...
        "note": "Structured layer only. The QA agent verifies each item against "
                ...
                "master records the result row in docs/grimoire/qa-ledger.md.",
    }
...
def _self_test():
    ...
    with tempfile.TemporaryDirectory() as d:
        os.makedirs(os.path.join(d, "docs", "grimoire"))
        ...
        with open(os.path.join(d, "docs", "grimoire", "qa-ledger.md"), "w") as fh:
        ...
        if "docs/grimoire/design/onboarding-design.md" not in sel.get("acceptance_sources", []):
            failures.append("grimoire/design acceptance source not extracted: %r" %
                            sel.get("acceptance_sources"))
    ...
    with tempfile.TemporaryDirectory() as d2:
        os.makedirs(os.path.join(d2, "docs", "grimoire"))
        with open(os.path.join(d2, "docs", "version-history.md"), "w") as fh:
            fh.write("## v3.1 — One\n\nbody\n")
        with open(os.path.join(d2, "docs", "grimoire", "qa-ledger.md"), "w") as fh:
    ...
    # Three-part version regression: parse_releases must yield full "vX.Y.Z" strings (#141).
    ...
    print("qa_select self-test: OK (release parse, ledger parse, earliest/"
          ...
          "all-verified-empty, missing-ledger degrade, three-part version (#141), "
          "determinism)")
    return 0
