...
holds the data. (Design: `docs/grimoire/design/model-effort-profiles-design.md`.)
...
Both `grm-release-phase` Step 3 and any agent choosing a tier resolve identically:
...
5. **UX pins (profile-invariant special-case):** `grm-design-language-adapt` →
   `sonnet`/`medium`, `grm-ux-demo-build` → `sonnet`/`high`, regardless of active
...
(e.g. `grm-release-phase` renders it into the spawned task name).
...
`docs/grimoire/design/token-efficiency-design.md` §Cache-aware authoring, lever 2):
...
Offer to create a design doc (using **`grm-design-doc-scaffold`**) if one doesn't
exist.
...
| Versioning & releases        | `docs/grimoire/version-design.md`                   |
| **Documentation conventions (wiki hierarchy, relative links, breadcrumbs)** | `docs/grimoire/README.md` § Wiki hierarchy & relative links |

For UX-tier work, see `grm-design-language-adapt` and `grm-ux-demo-build`.
