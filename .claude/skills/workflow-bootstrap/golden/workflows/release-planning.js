export const meta = {
  ...
  phases: [
    ...
    { title: 'Gather', detail: 'parallel readers (all haiku): roadmap, carryovers, design docs', model: 'haiku' },
...
const [roadmap, carry, design] = await parallel([
  () =>
    agent(
...
### 3. Work Items Summary  → master table: # | Area | Item | Est. Tokens, then a Total line.
### 4. Design Work Required  → table: Document | Status | What's needed (from DESIGN + any item with designStatus needs-design).
### 5. Observations for Iteration  → 3-6 bullets: scope risks, spike recommendations, velocity calibration, and any item the user must decide before lock.
Close with a one-line reminder that this is a planning input — next steps are design-doc-scaffold for §4 gaps, then release-agreement to lock scope.`,
