...
description: Build or update the versioned component registry .claude/component-registry.json from the project's component.json / front-matter sources — the scalable successor to the component-catalog-export stopgap. Reuses the same discovery, versions each component (declared version or content-hash), diffs against the prior registry (added/changed/removed/unchanged), and validates tags against the component-taxonomy authority (unknown tags surfaced, never silently accepted or dropped). Idempotent — unchanged sources produce a byte-identical registry. Triggers on "build the component registry", "update the component registry", "rebuild the registry", "regenerate component-registry.json", "diff the component registry", "version the components".
---
...
- Inventing metadata for a component that lacks `component.json` — record it
  under `uncataloged`.
- Writing a second discovery scanner — reuse the `component-catalog-export`
