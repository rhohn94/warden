...
Detailed formats, mode tables, and anti-patterns for the `cost-budget` skill.
...
authority: `docs/design/cost-governance-design.md`.
...
default-dispatch path and any scheduled routine). Interactive `release-phase`
...
| `verbose` | Full reasoning shown; context restated for auditability; longer summaries; tool-call descriptions may include rationale. Use only where correctness/auditability dominates cost (e.g. a `reviewer` or `researcher` that should show its work). |
...
The `priority-picker` skill (§F of the design doc) sets a verbosity default
...
- **Writing the dials.** The governance layer reads `work-paradigm`,
  ...
  **never writes them**. The `priority-picker` skill is the pure writer for the
