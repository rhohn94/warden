---
name: coding-practices-audit
description: Audit a codebase against the project's coding standards and architecture guidelines and produce a structured adherence report; optionally file one Issue Tracker issue per gap. Agent-driven (no linter/AST). The audit surface is read from machine-readable audit-hints in the standards docs, so it grows as those docs evolve — no skill change. Triggers on "audit coding practices", "check adherence to coding standards", "coding-practices audit", "audit this codebase against the standards", "file issues for standards gaps", "how well does this project follow our coding standards".
---
...
**`--file-issues`**, which files one issue per gap via `feedback-to-issue`.
...
Read the detected project type (the `workflow-bootstrap` GUI/project-type signal,
...
After the report, file **one issue per finding** via `feedback-to-issue`
...
- **Near-duplicate suppression:** before filing, check existing open issues; skip
  ...
  `feedback-to-issue`'s near-dup check plus the `id`-tagged title.
