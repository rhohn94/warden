...
Design authority: `docs/design/cost-governance-design.md` §F (priority-picker
...
Call `work-paradigm-switch` with the resolved paradigm value
(e.g. `Noir`, `Supervised`, `Weiss`).

### 6.2 Execution strategy

Call `workflow-variant-switch` with the resolved strategy
(`Fast`, `Efficient`, or `Cheap-Slow`).

### 6.3 Model/effort profile

Call `model-effort-profile-switch` with the resolved profile
...
>   - `work-paradigm-switch` — autonomy level
>   - `workflow-variant-switch` — dispatch shape
>   - `model-effort-profile-switch` — cost/quality tier
