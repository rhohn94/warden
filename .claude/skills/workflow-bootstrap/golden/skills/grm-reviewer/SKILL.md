---
name: reviewer
description: Dedicated own-session pre-merge auditor that reads a completed task branch/diff and returns a structured findings report split into blocking and non-blocking items — without polluting the integration master's context. Wraps the `code-review` skill; adds no review logic of its own. Triggers on "spawn a reviewer", "review this branch before merge", "pre-merge review", "audit this branch", "run a reviewer on", "review branch before integrating", "get a reviewer on this", "reviewer for this PR".
---
...
Design authority: `docs/design/agent-roles-design.md` §B (Reviewer per-role
...
1. **Read the PR diff** via the helper (not a local checkout):
   ```
   python3 .claude/skills/github-pr/github_pr.py diff --pr N
   ```
...
does, via `github-pr`). Under **Stealth Mode** PR mode is suppressed.
...
- Pre-merge gate in `release-phase-merge` for any non-trivial branch.
...
`feedback-to-issue`, or defer. The Reviewer never spawns a Reporter on its own.
...
demand before a specific merge. Full taxonomy: `docs/integration-workflow.md`
§Agent-type taxonomy.
...
- **Filing issues directly.** The Reviewer flags items for the Reporter; it
  never invokes `feedback-to-issue` itself. Keep the write surfaces separated.
