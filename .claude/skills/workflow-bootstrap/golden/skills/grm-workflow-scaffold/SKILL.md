...
description: Scaffold a new .claude/workflows/{name}.js Claude Code Workflow — either a read-only analysis fan-out (all paradigms) or a write-capable parallel-commit workflow (Noir only). Encodes the measured model-tiering and batch-vs-fanout cost lessons, plus the NW1 isolated-worktree safety rails. Triggers on "add a workflow", "scaffold a workflow", "create a Workflow for X skill", "mechanize this analysis step", "fan out this read step", "write-capable workflow", "parallel commit workflow".
---
...
rationale is `docs/design/release-planning-workflow-design.md`; the write-capable
tier design is `docs/design/write-capable-workflow-design.md` — point at them,
don't restate them.
...
`docs/design/write-capable-workflow-design.md §1.2` for the full spec.
...
- The integration master (not the Workflow itself) will own all merges into the
  staging ref via `release-phase-merge`.
...
Saved workflows live at `.claude/workflows/{name}.js` within the flavor root
(canonically `claude-code/.claude/workflows/`). The filename **is** the
...
5. **Minimize generated output — it's the most expensive token class, worst on
   Opus** (v1.9 lever 1; see `docs/design/token-efficiency-design.md`). Output is
...
`docs/design/write-capable-workflow-design.md §3`):
...
[`docs/token-efficiency-audit.md`](../../../docs/token-efficiency-audit.md) §D2).
...
Per-agent branches follow the work-item naming convention from `release-phase`:
...
`release-phase-merge`. On unresolvable conflict, the master surfaces a summary
to the user and pauses for resolution. See
`docs/design/write-capable-workflow-design.md §2.3`.
...
* **Skipping registration.** An un-manifested, un-goldened workflow isn't
  restorable — it'll vanish on the next `workflow-bootstrap` restore.
...
* **Skipping the conflict map / mergeAfter output.** The integration master needs
  the merge order to call `release-phase-merge` correctly. Always emit
