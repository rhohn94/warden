...
description: Bootstrap a project from a known-good application profile — match the declared app profile (API service, CLI, web app, …) against available quick-start templates and apply the closest match (scaffold its ready-made pieces + config defaults). Never overwrites existing files without confirmation. Triggers on "apply a quick-start template", "bootstrap from a template", "scaffold from a profile", "quick-start template", "use a starter template", "pick a template for this project", "start from an app profile".
---
...
persisted at onboarding / `workflow-bootstrap` Q9 / `web-app-apply` (§2.4), so
...
For the remaining profiles, read `workflow-bootstrap`'s detection signal (the
...
`component-catalog-export` report).
...
1. **Locate / refresh the matrix.** Read
   ...
   - If the cache is **absent or stale** — stale meaning its
     ...
     before reading (the `component-registry` skill owns regeneration).
...
1. Resolve the template's `components` against the catalog
   (`component-catalog-export`); warn on any unresolved id.
...
3. Write `config-defaults` via the existing switch skills
   (`model-effort-profile-switch`, `workflow-variant-switch`, etc.) — do not edit
...
captured at `workflow-bootstrap` feed the v1.26 merge gate — define each once,
reuse everywhere. See `docs/design/managed-project-tooling-design.md`.
