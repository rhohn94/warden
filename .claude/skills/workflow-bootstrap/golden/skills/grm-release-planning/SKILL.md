...
description: Generate a work-items report for the next planned release version. Reads design docs and release-planning history for context, assesses roadmap direction, surfaces carryovers from the in-flight release, and produces a structured report with token estimates and a list of design work required. Use when the user asks to "plan vX.Y", "generate the vX.Y work items", "start release planning for next version", "what's in vX.Y", or any forward-looking release scoping question. Triggers on phrases like "plan the next release", "what goes in vX.Y", "release planning", "preemptively plan".
---
...
> sized items, the integration master may run the `release-planning` *workflow*
> (`Workflow({ name: 'release-planning' })`, see
> `docs/integration-workflow.md` §Workflow-based orchestration) instead of
...
Also skim `docs/version-history.md` for the last 2–3 versions to calibrate
scope and velocity.
...
> `issue-tracker` is configured in `grimoire-config.json`) holds discrete filed
...
- **Missing — blocks implementation** — flag explicitly; the feature cannot
  start until the doc exists. Use the **`design-doc-scaffold`** skill to
  create it.
...
1. Use **`design-doc-scaffold`** for any missing design docs flagged in §4.
2. Run the **`release-agreement`** skill — it writes the planning doc, creates
