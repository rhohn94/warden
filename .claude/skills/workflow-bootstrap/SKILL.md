...
description: Guided install and restore of the agentic-workflow skill set. Detects missing or drifted workflow skills/hooks, restores them from self-contained golden copies, then runs an interactive interview to fill project-specific placeholders (test/build/release commands, version file, doc-location map, branch names, first roadmap entry) while preserving the workflow structure. Use when onboarding a new project, after copying this scaffolding into an existing repo, or when a required workflow skill is missing or corrupted. Triggers on "set up the workflow", "bootstrap skills", "restore skills", "install the agentic workflow", "my workflow skills are missing".
---
...
`workflow-snapshot` skill to re-baseline later.
...
- **settings.json** → if a live one exists and differs, do **not**
  ...
  allowlist (#72) so the framework's own maintenance scripts run unattended
  ...
  "permissions": {
    "allow": [
      "Bash($CLAUDE_PROJECT_DIR/.claude/skills/sync-from-upstream/sync-from-upstream.sh:*)",
      "Bash(.claude/skills/sync-from-upstream/sync-from-upstream.sh:*)",
      "Bash($CLAUDE_PROJECT_DIR/.claude/skills/sync-from-source/sync-from-source.sh:*)",
      "Bash(.claude/skills/sync-from-source/sync-from-source.sh:*)",
...
- **workflows** → `.claude/workflows/<name>.js` is a Claude-Code-only
  ...
  `docs/design/release-planning-workflow-design.md` for the path
...
- **paradigm content sets** → when `--restore` is requested (repair
  ...
  `work-paradigm-switch` (with no argument) to re-install the active
...
(`source-to-design-docs`, `design-doc-scaffold`, `design-language-adapt`) prefer
...
2. **`vendor.lock`** at the project root — the auto-generated resolved truth
   ...
   the `sync-deps` engine (`VendorLock.serialize`) emits — sorted keys, two-space
...
3. **`recipes.json`** — **ensure** the `sync-deps` + `vendor-check` targets are
...
4. **`.gitignore`** — **ensure** the line `.sync-deps-staging/` is present (the
   transient fixed staging dir the `sync-deps` engine creates and cleans per run;
...
3a. **Quality commands (optional, v1.26 merge gate)** — type-check
   ...
   `typecheck: build`). See `docs/design/merge-gate-quality-design.md`.
...
5. **Doc-location map** — the real paths for architecture / UX / feature
   / versioning docs (rows in `repo-reference`).
...
9. **GUI presence** — whether this project has (or will have) a user
   ...
   *Precedence (deterministic, highest wins):*
   ...
   (TUI)") so `ux-demo-build` can produce a stack-pure demo.
   ...
   *Confidence levels (changes presentation only — never removes the
   ...
   - **Yes** → ask two follow-up questions using `AskUserQuestion`:
     ...
     - *Primary GUI stack / framework hint*: the project's main GUI
       ...
       `ux-demo-build` to build a stack-pure demo.
     ...
     **Web-app persistence (v3.26 — `web-app-support-design.md` §2.4).** Once
     ...
     - **Web slice** — rows 8–13/15 (browser/meta web frameworks), corroborated
       ...
       the same write `web-app-apply` uses, no `schema-version` bump).
     ...
     makes the `quick-start-template` Step 1 project-type read **real**: after
   ...
   - **No, headless** → mark the UX tier N/A for this project. Note it
     ...
     `design-language-adapt` and `ux-demo-build` for this project's
...
| Test command  | `CLAUDE.md`, `release-phase`, `release-phase-merge` — every `{test-command}` |
| Build command | `CLAUDE.md`, `release-phase`, `release-phase-merge` — every `{build-command}` |
| Release command | `CLAUDE.md`, `project-release`, `docs/version-design.md` — every `{release-command}` |
...
| Version file + field | `docs/version-design.md` §3 — `{path/to/version/file}` / `{field name or format}` |
| Doc-location map | `repo-reference` SKILL.md — replace the map rows; keep the table shape |
...
| GUI: Yes → GUI stack hint | `docs/design/ux/design-language.md` §Design preamble — add a "Primary stack: `{ux-demo-stack}`" note for `ux-demo-build` to read; also echo in the Step 5 report |
...
| GUI: No, headless → N/A | Report only — no files changed; note that `design-language-adapt` and `ux-demo-build` are skipped for this project |
...
3. If `--restore` was run: confirm `.claude/paradigms/{supervised,weiss,noir}/`
   exist and are populated; confirm `work-paradigm-switch` ran to completion.
4. Report:
...
5. Suggest next steps: `source-to-design-docs` if `docs/design/` is empty,
   then `release-planning` when a release is being scoped.
