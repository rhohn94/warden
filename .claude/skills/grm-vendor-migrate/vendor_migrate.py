...
Resolution algorithm (design `dependency-channel-design.md` §7):
  ...
  2. Enumerate the published releases on the dep's channel; for each candidate,
     ...
     `grm-sync-deps` would (same ArtifactKind/strip/extract semantics), and compare
...
(the `grm-design-language-adapt` no-silent-clobber rule). Pass `--force` to
...
# DEP-CH-2 engine lives in the sibling `grm-sync-deps` skill; the copilot mirror
...
class VendorMigrator:
    ...
    def _loud_fallback(self, name, slug, channel, commit, content_sha, dirpath,
        ...
        banner = (
            ...
            "  fill the version/artifact, then run `grm-sync-deps` to lock it.\n"
            "==============================================================\n"
        )
