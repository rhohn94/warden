...
> deterministic stdlib engine that **reuses the DEP-CH-2 `grm-sync-deps` engine**;
> don't re-derive it in prose. Call the script and interpret its exit code:
>
> ```bash
> python3 .claude/skills/grm-vendor-migrate/vendor_migrate.py \
...
> python3 .claude/skills/grm-vendor-migrate/vendor_migrate.py --self-test
> ```
>
> Exit codes mirror the `grm-sync-deps` 0/1/2 contract: **0** = migrated (a release
...
2. **Derive the slug.** `owner/repo` is derived from the URL with the **same**
   normalization `grm-sync-deps` uses (`GhReleaseFetcher._slug`) — https or `git@`
...
3. **Content-match a release.** For each published release on the channel
   ...
   placement**, extract exactly as `grm-sync-deps` would (same `ArtifactKind` /
