...
workflow. It is read by the `grm-onboarding` skill's baseline-roadmap seeding step
(see `SKILL.md` §6.5 and `docs/grimoire/design/onboarding-design.md` §8), which selects
...
condition changes**, so a later onboarding re-run (or a `grm-sync-from-upstream`
...
inference) plus the test/build commands captured by `grm-workflow-bootstrap`:
...
| `gui-visual-inspection-cli` | GUI | Visual-inspection CLI (headless screenshot / render-to-file / DOM-or-scene dump / automation endpoint) — see UX tier (`grm-design-language-adapt`, `grm-ux-demo-build`) | A framework-required capability: an agent must be able to verify UI output without a human at the screen; cross-references the UX design tier rather than duplicating it. |
...
- The **GUI** row **cross-references** the UX tier (`grm-design-language-adapt` →
  `docs/design/ux/design-language.md`, `grm-ux-demo-build` → `ux-demo/`); the
  ...
  GUI-deferred project, `grm-repo-init` already adds a `## Backlog` UX row; this
...
- The **web-app** rows (`web-app-healthz`, `web-app-deploy-bundle`,
  ...
  (`docs/web-app-deployment-protocol.md`) and are the rows the `grm-web-app-apply`
