---
name: grm-verifier
description: Dedicated own-session QA agent that checks a completed work-item branch against its acceptance criteria — running tests, build, and release commands — and returns a structured pass/fail verdict. Distinct from the Reviewer (reads code) and the task agent (self-reviews). Use when the user wants to spawn a verifier or run independent pre-merge QA on a branch.
---
