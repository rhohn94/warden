...
per-flavor `.zip`s stay (the `grm-sync-from-upstream` transport selects + verifies
...
# Framework-internal doc paths that must NEVER ship to a consumer (v3.39
# "Bulkhead" / documentation-separation-design.md §4). These are multi-segment
# posix PREFIXES matched against a file's flavor-relative path with
# `startswith` — EXCLUDED_COMPONENTS only matches single path components, so a
# nested prefix like `docs/grimoire/design/` slips through it. This check is
# ADDITIVE: both filters run in _collect_files(). The single-file entries are
# exact paths (also matched by startswith); `docs/grimoire/design/` is a whole
# subtree. `docs/grimoire/README.md` is deliberately NOT listed — it stays
# shipped (it carries the consumer-facing wiki-convention authority, §6).
# `maintaining-grimoire.md` is root-only (absent in claude-code); listed for
# defense/consistency across flavors.
EXCLUDED_PATH_PREFIXES = (
    "docs/grimoire/design/",                        # whole framework design subtree
    ...
    "docs/grimoire/release-planning-",              # relocated historical release plans (prefix)
    # ── release-planning carve-out (CR-2 guardrail): the ACTIVE in-flight plan
    #    docs/release-planning-v{X.Y}.md is path-coupled to the release machinery
    #    + release-plan-guard hook and must NOT move. Excluding the top-level
...
class DistributableBuilder:
    ...
    def _is_excluded(self, rel) -> bool:
        """True if the flavor-relative path is excluded from a distributable.

        Two ADDITIVE filters (v3.39): (1) any path COMPONENT in
        EXCLUDED_COMPONENTS (VCS noise / derived dirs), and (2) the file's posix
        path startswith any framework-internal prefix in EXCLUDED_PATH_PREFIXES
        (documentation-separation-design.md §4). `rel` is a pathlib.Path
        relative to the flavor dir.
        """
        if any(part in EXCLUDED_COMPONENTS for part in rel.parts):
            return True
        posix = rel.as_posix()
        return any(posix.startswith(prefix) for prefix in EXCLUDED_PATH_PREFIXES)
...
def _self_test() -> int:
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        for name in (CANONICAL_FLAVOR, "flavor-b"):
            ...
            # v3.39 documentation-separation: framework-internal docs must be
            # excluded by EXCLUDED_PATH_PREFIXES; shipped docs must remain.
            (d / "docs" / "grimoire" / "design").mkdir(parents=True)
            (d / "docs" / "grimoire" / "design" / "foo-design.md").write_text("# foo\n")   # excluded (subtree)
            (d / "docs" / "grimoire" / "README.md").write_text("# tier index\n")            # KEPT (shipped)
            (d / "docs" / "grimoire" / "sync-flow-audit.md").write_text("# audit\n")        # excluded (named)
            # v3.41 CR-2: relocated operational docs + top-level carve-outs.
            (d / "docs" / "grimoire" / "integration-workflow.md").write_text("# iw\n")      # excluded (relocated)
            (d / "docs" / "grimoire" / "token-efficiency-audit.md").write_text("# te\n")    # excluded (prefix)
            (d / "docs" / "grimoire" / "release-planning-v1.0.md").write_text("# rp\n")     # excluded (prefix, relocated)
            (d / "docs" / "release-planning-v9.9.md").write_text("# active\n")              # excluded (top-level carve-out)
        ...
        # v3.39 documentation-separation (EXCLUDED_PATH_PREFIXES):
        check(
            f"{CANONICAL_FLAVOR}/docs/grimoire/design/foo-design.md" not in names,
            "docs/grimoire/design/ subtree excluded from distributable",
        )
        check(
            f"{CANONICAL_FLAVOR}/docs/grimoire/sync-flow-audit.md" not in names,
            "named framework-internal study artifact excluded",
        )
        check(
            f"{CANONICAL_FLAVOR}/docs/grimoire/README.md" in names,
            "docs/grimoire/README.md (tier index) is NOT excluded — stays shipped",
        )
        check(
            f"{CANONICAL_FLAVOR}/docs/design/bar-design.md" in names,
            "docs/design/ (project-own) design doc is NOT excluded — stays shipped",
        )

        # v3.41 "Clean-Room" CR-2 (clean-room-design §4): relocated operational
        # docs + the release-planning carve-out + version-history exclude-and-seed.
        for excluded_rel, why in (
            ("docs/grimoire/integration-workflow.md", "relocated operational doc excluded"),
            ("docs/grimoire/token-efficiency-audit.md", "token-efficiency- prefix excluded"),
            ("docs/grimoire/release-planning-v1.0.md", "relocated release-planning prefix excluded"),
            ("docs/grimoire/release-planning-v9.9.md", "top-level active release-plan carve-out excluded"),
            ("docs/version-history.md", "version-history (Grimoire log) excluded — consumer gets seed"),
        ):
            check(f"{CANONICAL_FLAVOR}/{excluded_rel}" not in names, why)
