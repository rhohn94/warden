---
name: grm-repo-init
description: Initialize a new git repository with this project's branch model, commit discipline, and push protection. Sets up `main` + `dev`, documents the `version/*` → `dev` integration hierarchy, enforces atomic one-sentence commits with no Co-Authored-By trailer, and installs the no-push guard so only a human can push. Use when starting a brand-new repo, running `git init`, or setting up version control / the branch structure for a project.
---
...
For installing/restoring the *workflow skill set* use `grm-workflow-bootstrap`;
for promoting `dev` → `main` at release time use `grm-project-release`. This
...
- **`main` only ever receives release merges**, never direct work. Each such
  commit gets a version tag (`v<number>`) — see `grm-project-release`.
...
> (`grm-release-agreement`, `grm-release-phase-merge`, …) cut, merge, and tear down
...
0. **Require an existing repo (fail-soft guard).** `grm-repo-init` no longer
   creates a repository from nothing — it builds the branch model on top of one
   that already exists. Repo *creation* is owned by the `grm-onboarding` skill
   (which detects a non-git dir, confirms with the user, and runs `git init` +
   an initial scaffold commit before calling `grm-repo-init`). Defence in depth: if
   `grm-repo-init` is invoked standalone in a non-git directory, stop with guidance
   and mutate **nothing**.
...
6. **UX design language (optional).** Choose the branch that matches the
   ...
   - **GUI project (has a GUI now):** run `grm-design-language-adapt` to establish
     ...
     is listed in `.gitignore` (the skill appends it if absent). `grm-ux-demo-build`
   ...
   - **GUI deferred (will have a GUI later):** add one row under `## Backlog`
     ...
     **Configured tracker note:** if `.claude/grimoire-config.json` already
     contains an `grm-issue-tracker` block with a non-roadmap provider (e.g.
   ...
   - **Headless / never-GUI:** skip this step entirely. Note in `grm-repo-reference`
     that the `docs/design/ux/` tier is N/A for this project;
     `grm-design-language-adapt` and `grm-ux-demo-build` are not applicable.
...
the end of `grm-project-release` (pushing `dev` + `main` + tag together).
`grm-release-phase-merge` no longer pushes. Outside that moment, don't push. Full
policy: `docs/grimoire/integration-workflow.md` §Pushing to origin.
...
- Running `grm-repo-init` in a non-git directory expecting it to bootstrap a repo
  from scratch — it no longer does. The fail-soft guard (step 0) stops and
  defers repo *creation* to the `grm-onboarding` skill; `grm-repo-init` only builds the
...
- Seeding a `## Backlog` issue-tracking section in `docs/roadmap.md` when a
  non-roadmap `grm-issue-tracker` provider is already configured — seed a stub or
