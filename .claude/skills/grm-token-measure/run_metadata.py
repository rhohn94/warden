...
`grm-token-measure` only *prints* a token table and the release ledgers are markdown
...
sibling `grm-token-measure` helper `parse_usage.py` (the single source of truth for
...
class TokenSplit:
    ...
    `grm-token-measure` session parse (see `from_transcript`), and degrades to
