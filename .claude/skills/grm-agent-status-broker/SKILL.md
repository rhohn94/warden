...
**See also:** `grm-agent-scout` — both are strictly read-only investigation
agents, but for different questions. Use the **status-broker** for a "what is
the status of X" lookup (project/feature/release state), answered cheaply via
issue tracker → docs → code, in that order. Use the **Scout** for a bounded,
open-ended investigative question ("how does X work", "what's the prior art
for Y") that isn't a status check.
...
Design rationale and the scripting standard are framework-internal — see the
...
```
python3 .claude/skills/grm-issue-tracker/issue_tracker.py list --state all   # or: search "<term>"
```
...
>
> ```
> python3 .claude/skills/grm-agent-status-broker/project_status.py --root .
> ```
>
> Both paths emit the same fields; check `degraded` to know which sources were
> missing. (Design rationale in the upstream Grimoire repository,
> framework-internal.)
...
the tree. For a single release's notes, run
`python3 .claude/skills/grm-agent-status-broker/version_history.py --release vX.Y`
(`--latest` / `--list`) instead of reading the >100 KB `version-history.md` whole.
...
`grm-agent-qa/qa_select.py` imports `parse_releases` directly from this
skill's `project_status.py` (#342) rather than re-deriving the version-heading
regex. `project_status.py` declares `parse_releases` in its `__all__` and
marks it as consumed by grm-agent-qa — do not rename it here without updating
that consumer (#351).
