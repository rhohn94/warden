...
# Public surface. `parse_releases` is consumed cross-skill by
# grm-agent-qa/qa_select.py (#342) — do not rename without updating that
# consumer (see qa_select.py's sys.path/import comment).
__all__ = [
    "read_config",
    "parse_releases",
    "parse_in_flight",
...
def read_config(root: str) -> dict | None:
...
def parse_releases(version_history_text: str | None) -> list:
...
def parse_in_flight(roadmap_text: str | None, shipped_versions: set) -> dict | None:
...
def parse_manifest_version(manifest_text: str | None) -> int | None:
...
def detect_tech_stack(root: str) -> list:
