...
Usage:
  ...
  version_history.py --file changelog.md  # slice docs/changelog.md instead
                                           # (the front-facing, release-notes file)
...
def default_path(root: str, filename: str = "version-history.md") -> str:
    """The conventional location of the version-history (or changelog) file under a project root."""
    return os.path.join(root, "docs", filename)


def parse_sections(text: str | None) -> list:
...
def get_release(sections: list, version: str) -> dict | None:
...
def list_lines(sections: list) -> list:
...
def _load(root, filename="version-history.md"):
    path = default_path(root, filename)
    text = _read(path)
    if text is None:
        print("error: %s not found: %s" % (filename, path), file=sys.stderr)
        return None, None
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument("--file", default="version-history.md",
                    help="docs/ filename to slice (default: version-history.md; "
                         "pass changelog.md to extract front-facing release notes)")
    ...
    path, sections = _load(args.root, args.file)
    if sections is None:
        return 2
