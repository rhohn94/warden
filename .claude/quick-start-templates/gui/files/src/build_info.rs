...
// `--version`/`-V` to REAL build provenance instead of only the crate's
...
//
// This is the SAME module shipped in the cli/service quick-start templates
// (self-contained per-template copy — no shared crate across profiles, matching
// the seam-file convention already used by the web quick-start template).
