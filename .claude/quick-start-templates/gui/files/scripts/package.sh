...
# `cargo build --release` is this fleet's single largest disk consumer
# (docs/grimoire/design/disk-branch-hygiene-design.md §Release-build
...
# both accumulate a --release build's disk cost with nothing to reclaim it.
# Reuses the SAME "spawned task-agent worktree" check pattern
# protected-branch-guard.sh's `worktree_of()` uses (a path segment under
# .claude/worktrees/<id>/, unmarked by integration-allow.local) rather than
# inventing a new heuristic. LOUD, not silent (this repo's existing
...
run_package() {
    ...
    if [ "${PACKAGE_SKIP_BUILD:-0}" != "1" ]; then
        [ -f Cargo.toml ] && warn_if_not_throwaway
    ...
    [ -f "$out_tar" ] || die "packaging failed: $out_tar was not produced"
    note "wrote $out_tar"

    # 6. Reclaim disk (#448): cargo build --release is this fleet's single
    #    largest disk consumer. Now that the packaged artifact is produced and
    #    verified present (the tarball above), the target/release/ tree that
    #    produced it has served its purpose for this packaging run — remove
    #    it immediately rather than waiting for a later sweep. Only
    #    target/release/ (never the whole target/) — target/debug/ (dev
    #    builds) is left untouched; #447's sibling item shrinks that
    #    separately.
    if [ -f Cargo.toml ] && [ -d target/release ]; then
        note "reclaiming disk: removing target/release/ (packaged artifact verified at $out_tar)"
