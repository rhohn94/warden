...
# ── release-build throwaway-context guard (#448) ───────────────────────────
# `cargo build --release` (triggered here via the ceremony's own build_cmd,
# e.g. `recipe.py build --env prod`) is this fleet's single largest disk
# consumer (docs/grimoire/design/disk-branch-hygiene-design.md §Release-build
# throwaway-context guard). It should only run in a THROWAWAY worktree —
# never the canonical (non-worktree) checkout, and never a long-lived
# integration-master worktree (marked by .claude/integration-allow.local) —
# both are long-lived, so a --release build there accumulates disk with
# nothing to reclaim it. Reuses the SAME "spawned task-agent worktree" check
# pattern protected-branch-guard.sh's `worktree_of()` uses (a path segment
# under .claude/worktrees/<id>/, unmarked by integration-allow.local) rather
# than inventing a new heuristic. Mirrors assert_release_profile's own
# Cargo.toml + prod-build gate immediately above (only a Cargo project's prod
# build actually invokes --release). LOUD, not silent (this repo's existing
# degrade-loud convention, e.g. grm-project-release's publish-step behavior)
# — never a hard failure, since a human may have a legitimate reason (e.g.
# reproducing a release build locally in the canonical checkout).
warn_if_not_throwaway() {
    local build_cmd="$1"
    [ -f Cargo.toml ] || return 0
    case "$build_cmd" in
        *--env\ prod*|*--env=prod*) ;;
        *) return 0 ;;  # not a prod build — nothing to warn about
    esac
    case "$(pwd -P)" in
        */.claude/worktrees/*)
            if [ -f ".claude/integration-allow.local" ]; then
                note "WARNING: this looks like a long-lived integration-master worktree"
                note "         (.claude/integration-allow.local present) — a 'cargo build"
                note "         --release' here is a long-lived disk cost, not a throwaway"
                note "         one. Proceeding anyway (a human may have a legitimate reason)."
            fi
            ;;
        *)
            note "WARNING: this does not look like a throwaway (.claude/worktrees/) checkout"
            note "         — 'cargo build --release' here is this fleet's single largest disk"
            note "         consumer and won't be reclaimed by a spawned agent's worktree"
            note "         teardown. Proceeding anyway (a human may have a legitimate reason)."
            ;;
    esac
}

# ── target/release cleanup (#448) ──────────────────────────────────────────
# Once the ceremony's own build step above (and the archive step's package
# recipe) have produced and archived the release artifact, the
# target/release/ tree that produced it has served its purpose for this run —
# reclaim it immediately rather than waiting for a later sweep. Only fires
# when THIS run actually built --release (the caller only calls this after a
# real, non-skipped build step); a project with no Cargo.toml is a no-op.
# Never the whole target/ — target/debug/ (dev builds) is left untouched;
# #447's sibling item shrinks that separately.
reclaim_release_build() {
    [ -f Cargo.toml ] || return 0
    if [ "${DRY_RUN:-0}" = "1" ]; then
        echo "  [dry-run] rm -rf target/release (reclaim disk after packaging, #448)"
        return 0
    fi
    if [ -d target/release ]; then
        note "reclaiming disk: removing target/release/ (packaged artifact archived above, #448)"
...
run_release() {
    ...
    if [ -n "$cli_no_verify" ] || [ "${RELEASE_SKIP_VERIFY:-0}" = "1" ]; then
        ...
        warn_if_not_throwaway "$build_cmd"
    ...
    # #448: reclaim target/release/ now that this run's own build (if any)
    # has been archived above — package.sh (gui/service/cli) also does this
    # cleanup on its own direct-invocation path; this is idempotent with that.
    [ "$did_build" = "1" ] && reclaim_release_build
